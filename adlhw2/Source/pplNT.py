...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to a PEFT checkpoint. If omitted the base model is used.",
    )
    parser.add_argument(
        "--test_data_path",
        type=str,
    ...
    # Optionally load PEFT adapters when a checkpoint is provided
    if args.peft_path:
        try:
            from peft import PeftModel
        except ImportError as exc:
            raise ImportError(
                "PEFT is required to load adapters but is not installed. "
                "Install the `peft` package or omit --peft_path to run the base model."
            ) from exc
